...
export const authService = {
  ...
  async register(
    ...
    try {
      ...
      if (!user) {
        return { success: false, error: 'Registration failed. No user was returned from Supabase.' };
      }

      // 2. Create row in public.profiles
      const now = new Date().toISOString();
      ...
      if (role === 'farmer') {
        ...
        const { error: fErr } = await supabase
          .from('farmer_profiles')
          .upsert(roleDataRecord);
        if (fErr) console.error('Error inserting farmer_profiles:', fErr);
        ...
        const { error: dErr } = await supabase
          .from('distributor_profiles')
          .upsert(roleDataRecord);
        if (dErr) console.error('Error inserting distributor_profiles:', dErr);
        ...
        const { error: tErr } = await supabase
          .from('transporter_profiles')
          .upsert(roleDataRecord);
        if (tErr) console.error('Error inserting transporter_profiles:', tErr);
      } else if (role === 'consumer') {
        ...
        const { error: cErr } = await supabase
          .from('consumer_profiles')
          .upsert(roleDataRecord);
        if (cErr) console.error('Error inserting consumer_profiles:', cErr);
      }
